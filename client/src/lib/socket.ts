...
import type { GameState, Player, Song, RoundResult } from '@/types/game.types';
...
class SocketService {
  ...
  createGame(
    callback: (data: { gameId: string; gameState: GameState }) => void,
    mode: 'local' | 'online' = 'local'
  ) {
    if (!this.socket) return;
    this.socket.emit('createGame', { mode });
